...
hl.bind(mainMod .. " + D",           hl.dsp.window.float({ action = "toggle" })) -- Detach from tiling (free move/resize)
hl.bind(mainMod .. " + F",           hl.dsp.window.fullscreen())
hl.bind(mainMod .. " + ALT + F",     hl.dsp.window.fullscreen({ mode = 1 })) -- Maximize (moved off SUPER+D)
...
hl.bind(mainMod .. " + ALT + Up",                     hl.dsp.window.move({ direction = "u" })) -- moved off SHIFT+arrow
...
-- Move active window to a virtual screen (workspace) by number, following it with focus
for i = 1, 10 do
    local key = i % 10
    hl.bind(mainMod .. " + SHIFT + " .. key, hl.dsp.window.move({ workspace = i, follow = true }))
end

-- Move active window to a virtual screen (workspace) by number, WITHOUT switching the visible screen
for i = 1, 10 do
    local key = i % 10
    hl.bind(mainMod .. " + CONTROL + " .. key, hl.dsp.window.move({ workspace = i, follow = false }))
end

-- Resize active window towards the pressed direction
local resizeStep = 30
...
-- Focus on a virtual screen (workspace) by number
for i = 1, 10 do
    local key = i % 10
    hl.bind(mainMod .. " + " .. key, hl.dsp.focus({ workspace = i }))
end
-- Absolute (kept as an alias with ALT for compatibility)
for i = 1, NUM_WPM do
    local key = i % 10
